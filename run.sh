...
awk -f enqueue.awk ${mode}_out.tr > ${mode}_q.txt
...
gnuplot -p -e "set xrange [0.2:3.2]; set xtics 0,0.2,3;set xlabel 'Time(s)';set ylabel 'Packet # (mod 60)'; set title '$mode'; set datafile missing '0.0000';plot  '${mode}_q.txt' using 2:1 linestyle 9,'${mode}_pktnum.txt' using 2:1 linestyle 8, '${mode}_ack.txt' using 2:1 linestyle 2"
